...
(function () {
  ...
  let activeCubeInstances = []; // track 3D cubes for cleanup
  ...
  // Navigation
  // ========================================
  /** Clean up active 3D cube instances (stop auto-rotate intervals) */
  function cleanupCubes() {
    activeCubeInstances.forEach(inst => {
      if (inst && inst.autoRotateId) {
  ...
  function renderIntro() {
    ...
    setTimeout(() => {
      const inst = Cube3D.create('introCube');
      if (inst) activeCubeInstances.push(inst);
    }, 50);
  }
  ...
  function renderStep(stepId) {
    ...
    mainContent.innerHTML = `
      <div class="step-page">
        ...
        <!-- Diagram -->
        ${(step.diagram || step.cube3d) ? renderDiagram(step.diagram, strings, step.cube3d) : ''}
  ...
  function renderDiagram(diagram, strings, cube3d) {
    let cube3dHtml = '';
    if (cube3d) {
      const cubeId = 'stepCube3d_' + Date.now();
      cube3dHtml = `
        <div class="step-3d-container">
          <div id="${cubeId}" class="cube-3d-container"></div>
          <p class="cube-hint">${currentLang === 'he' ? 'גרור לסיבוב' : 'Drag to rotate'}</p>
        </div>
      `;
      // Schedule 3D cube creation after DOM insertion
      setTimeout(() => {
        const inst = Cube3D.createCube(cubeId, {
          colors: cube3d.colors,
          rotX: cube3d.rotX,
          rotY: cube3d.rotY,
          autoRotate: false,
          draggable: true,
    ...
    if (!diagram || !diagram.cells) {
      // No 2D diagram but we might have a 3D cube
      if (cube3dHtml) {
        return `
          <div class="cube-diagram-container">
            <h2>${escHtml(strings.whatItLooksLike)}</h2>
  ...
  function renderPatterns(patterns, strings) {
    ...
    // Generate unique IDs for pattern 3D cubes
    const ts = Date.now();
    const patternCubeIds = patterns.map((p, i) => p.cube3d ? `patCube3d_${ts}_${i}` : null);
        ...
        <div class="patterns-grid">
          ${patterns.map((p, i) => `
            <div class="pattern-card">
              ...
              ${patternCubeIds[i] ? `<div id="${patternCubeIds[i]}" class="pattern-3d"></div>` : ''}
    ...
    // Schedule mini 3D cube creation after DOM insertion
    setTimeout(() => {
      patterns.forEach((p, i) => {
        if (p.cube3d && patternCubeIds[i]) {
          const inst = Cube3D.createCube(patternCubeIds[i], {
            colors: p.cube3d.colors,
            rotX: p.cube3d.rotX,
            rotY: p.cube3d.rotY,
            autoRotate: false,
            draggable: true,
